...
def cluster():
    ...
    # 概念层（众包特征聚类）
    #
    concept_estimator = KMeans(n_clusters=2)  # 构造聚类器
    ...
    # 物理层（测试集特征聚类）
    #
    physical_estimator = KMeans(n_clusters=2)  # 构造聚类器
    ...
    Dp0 = 0
    for i in np.where(physical_label_pred == 0)[0]:
        for j in np.where(physical_label_pred == 0)[0]:
            Dp0 += np.linalg.norm(X_test[i] - X_test[j])
    Dp0 /= len(np.where(physical_label_pred == 0)[0])
    Dp0 /= len(np.where(physical_label_pred == 0)[0])
    print('Dp0:' + str(Dp0))
    Dp1 = 0
    for i in np.where(physical_label_pred == 1)[0]:
        for j in np.where(physical_label_pred == 1)[0]:
            Dp1 += np.linalg.norm(X_test[i] - X_test[j])
    Dp1 /= len(np.where(physical_label_pred == 1)[0])
    Dp1 /= len(np.where(physical_label_pred == 1)[0])
    print('Dp1:' + str(Dp1))
    ...
    for i in np.where(conceptLayer_label_pred == 0)[0]:
        for j in np.where(conceptLayer_label_pred == 0)[0]:
            Dc0 += np.linalg.norm(preAll[i] - preAll[j])
    Dc0 /= len(np.where(conceptLayer_label_pred == 0)[0])
    Dc0 /= len(np.where(conceptLayer_label_pred == 0)[0])
    print('Dc0:' + str(Dc0))
    Dc1 = 0
    # print('here')
    ...
    for i in np.where(conceptLayer_label_pred == 1)[0]:
        for j in np.where(conceptLayer_label_pred == 1)[0]:
            Dc1 += np.linalg.norm(preAll[i] - preAll[j])
    Dc1 /= len(np.where(conceptLayer_label_pred == 1)[0])
    Dc1 /= len(np.where(conceptLayer_label_pred == 1)[0])
    print('Dc1:' + str(Dc1))
    ...
    wrong = 0
    for i in uncertain:
        if ((physical_label_pred[i] == real[i]) & (real[i] != list[i])):
            correct += 1
        if ((physical_label_pred[i] == list[i]) & (real[i] != list[i])):
            bubian += 1
        if ((physical_label_pred[i] != real[i]) & (real[i] == list[i])):
            wrong += 1
