...
def cluster():
    ...
    # 概念层（众包特征聚类）
    #
    concept_estimator = Birch(n_clusters=2)  # 构造聚类器
    # concept_estimator.fit(Xitaarr)  # 聚类
    concept_estimator.fit_predict(preAll)  # 聚类
    # concept_estimator.fit(preAllnmf)  # 聚类
    conceptLayer_label_pred = concept_estimator.labels_  # 获取聚类标签
    concept_cluster_centers = concept_estimator.subcluster_centers_  # 获取聚类标签
    ...
    # 物理层（测试集特征聚类）
    #
    physical_estimator = Birch(n_clusters=2)  # 构造聚类器
    physical_estimator.fit_predict(X_test)  # 聚类
    physical_label_pred = physical_estimator.labels_  # 获取聚类标签
    physical_cluster_centers = physical_estimator.subcluster_centers_  # 获取聚类标签
    ...
    print("correct,haicuo,wrong，haidui:{},{},{}.{}".format(correct, haicuo, wrong,haidui))
